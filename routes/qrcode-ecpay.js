...
const OrderResultURL = process.env.ECPAY_ORDER_RESULT_URL_QRCODE
const ReactClientBackURL = process.env.ECPAY_ORDER_CALLBACK_URL_QRCODE
...
router.get('/payment', async (req, res, next) => {
  ...
  // 從資料庫取得訂單資料
  const orderRecord = await Qrcode_Ecpay.findByPk(orderId, {
    raw: true, // 只需要資料表中資料
  })
  ...
  const MerchantTradeNo =
    ...
    crypto.randomBytes(8).toString('hex').substring(0, 6)
